...
async fn wait_for_container_statistic (pod_name: &str, wait_time: u64) ->  anyhow::Result<f64> {
    ...
    loop {
        ...
        .output()
        .await.map_err(|e| anyhow::Error::msg(format!("wait_for_container_statistic failed to run micro benchmark, get error {:?}", e)))?;
...
pub async fn perf_ram (elf_size : i32, loop_time: i32, my_logger: &mut MyLoger<File>, image_name: &str, runtime_type: &RuntimeType) ->  anyhow::Result<()> {
    ...
    let durantion_statistic = get_statistic(&read_vec_duration).unwrap();


    info!("runtime type {:?}, elf size {:?} image_nmae {:?} durantion_statistic {:?}", runtime_type, elf_size, image_name, durantion_statistic);
